...
function langSet(langId) {
  $.ajax({
    url: "language-setting.php",
    type: "POST",
    data: {
      lang_id: langId
    },
...
function configValueUpdate(configId, value) {
  $.ajax({
    url: "assets/php/config-value-update.php",
    type: "POST",
    dataType: "JSON",
    data: {
      config_id: configId,
...
function configValueUpdateConfirm(configId, value) {
  $("#confirm_header").html("Confirmation");
  $("#confirm_body").html("Are you sure you want to update this configure setting?");
  $("#confirm_btn").attr("onClick", "configValueUpdate(" + configId + "," + value + ")");
...
function numberFormatter(inputId, length) {
  ...
  for (let i = 0; i < amount.length; i++) {
    let ascii = amount.charCodeAt(i);
    if (ascii >= 48 && ascii <= 57) {
      asciiArr.push(ascii);
    }
  }
  amount = String.fromCharCode.apply(null, asciiArr);
  if (amount.length > length) {
    amount = amount.substring(0, length);
  }
  $(inputId).val(amount);
}
function packageUpdateConfirm(packageId) {
  $(".modal").modal("hide");
  $("#confirm_header").html("Confirmation");
  $("#confirm_body").html("Are you sure you want to update this package values?");
  $("#confirm_btn").attr("onClick", "packageUpdate(" + packageId + ")");
  $("#confirm_modal").modal("show");
}
function packageUpdate(packageId) {
  let amount = $("#input_amount").val();
  let price = $("#input_price").val();
  $.ajax({
    url: "assets/php/package-update.php",
    type: "POST",
    dataType: "JSON",
    data: {
...
function registration(operType) {
  let name = $("#name").val();
  let email = $("#email").val();
  ...
  $.ajax({
    url: "assets/php/registration-operation.php",
    type: "POST",
    dataType: "JSON",
    data: {
      name: name,
    ...
    success: function (response) {
      ...
      if (dataStatus > 0) {
        if (operType == 0) {
          $("#confirm_header").html("Confirmation");
          $("#confirm_body").html("Are you sure you want to register this new account?");
          $("#confirm_btn").attr("onClick", "registration(1)");
          $("#confirm_modal").modal("show");
        }
        if (operType == 1) {
          $("#inform_success_body").html(response.return);
          $("#inform_success_btn").attr("onClick", "window.location.replace('login.php')");
          $("#inform_success").modal("show");
        }
      }
      if (dataStatus < 0) {
        $("#inform_danger_body").html(response.return);
        $("#inform_danger_btn").attr("onClick", "$('.modal').modal('hide')");
        $("#inform_danger").modal("show");
      }
    },
    error: function (response) {
...
function signInOperation() {
  ...
  let password = $("#password").val();
  $.ajax({
    url: "assets/php/sign-in-operation.php",
    type: "POST",
    dataType: "JSON",
    data: {
      email: email,
    ...
    success: function (response) {
      ...
      if (dataStatus == -1) {
        $("#sign_in_err").html(response.return);
      }
      if (dataStatus == 1) {
...
function nameFormat(inputId) {
  let tmpName = $(inputId).val();
  let asciiArr = [];
  for (let i = 0; i < tmpName.length; i++) {
    let ascii = tmpName.charCodeAt(i);
    if ((ascii >= 65 && ascii <= 90) || (ascii >= 97 && ascii <= 122) || ascii == 32 || ascii == 47 || ascii == 46) {
      if (ascii == 32 || ascii == 47 || ascii == 46) {
        if (i != 0) {
          if ((asciiArr[i - 1] >= 65 && asciiArr[i - 1] <= 90) || (asciiArr[i - 1] >= 97 && asciiArr[i - 1] <= 122)) {
            asciiArr.push(ascii);
          }
        }
      } else {
  ...
  tmpName = String.fromCharCode.apply(null, asciiArr);
  if (tmpName.length > 40) {
    tmpName = tmpName.substring(0, 40);
  }
  $(inputId).val(tmpName);
}